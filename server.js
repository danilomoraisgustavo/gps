...
app.get('/positions', (req, res) => {
    if (!req.session.user) { res.json([]); return }
    db.all(`SELECT p.* FROM positions p INNER JOIN devices d ON p.deviceId=d.id WHERE d.userId=? AND p.deviceId=? ORDER BY p.id DESC LIMIT 50`, [req.session.user.id, req.query.deviceId], (err, rows) => {
        res.json(rows || [])
    })
})
    ...
    socket.on('data', (buf) => {
        ...
        if (p && p.type === 'location') {
            db.all(`SELECT id,imei FROM devices`, (err, rows) => {
                ...
                if (dev) {
                    db.run(`INSERT INTO positions (deviceId,timestamp,latitude,longitude,speed) VALUES (?,?,?,?,?)`, [dev.id, new Date(), p.lat, p.lon, p.speed])
                }
            })
